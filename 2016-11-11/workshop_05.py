...
def ggpl_chair(dx,dy,dz):
	...
	#creates all four legs
	legFront = CYLINDER([r,dz/2])(100)
	legFront = COLOR(GRAY)(legFront)
	legBack = CYLINDER([r,(dz*3)/4])(100)
	...
	chair = STRUCT([chair,T([1,2,3])([-r,-r,dz/3]),sit])

	#creates the back of the chair
	back = CUBOID([.05,dy+(r*2),dz/4])
	back = COLOR(Color4f([205/255.0,170/255.0,125/255.0,1]))(back)
	chair = STRUCT([chair,T([1,2,3])([-r,-r,(dz*3)/4]),back])
...
def ggpl_classroom(dx,dy,dz):
	...
	while dy - deskDist > count2 + deskY :
...
def main():
	#ggpl_blackboard(2.5,1,.1)
	#ggpl_desk(1.2,0.7,0.9)
	#ggpl_chair(0.5,0.6,1.2)
	#ggpl_desk_chair(1.2,0.7,0.9)
	ggpl_classroom(20,10,3.5)
